...
class FileAnalysisService {
  ...
  analyzeRepositoryStructure(files) {
    ...
    files.forEach(file => {
      ...
      if (fileInfo.category) {
        analysis.filesByCategory[fileInfo.category] = (analysis.filesByCategory[fileInfo.category] || 0) + 1;
  ...
  shouldIgnoreFile(filePath, fileName) {
    ...
    for (const ignoredPath of this.ignoredPaths) {
      if (ignoredPath.includes('*')) {
        ...
        if (filePath.includes(ignoredPath) || fileName === ignoredPath) {
          return true;
        }
      }
    }
  ...
  detectProjectStructure(files) {
    ...
    const fileNames = files.map(f => f.name || path.basename(f.path));
    ...
    // Detect Python
    else if (fileNames.includes('requirements.txt') || fileNames.includes('Pipfile') || fileNames.includes('pyproject.toml')) {
  ...
  selectFilesForTestGeneration(files, maxFiles = 10) {
    ...
    let selectedFiles = analysis.recommendedFilesForAnalysis
      .filter(file => file.category === 'source' || 
                     (file.category === 'config' && file.importance === 'high'))
      .slice(0, maxFiles);
    ...
    if (selectedFiles.length < maxFiles && analysis.testFiles.length > 0) {
      ...
      selectedFiles = selectedFiles.concat(
        analysis.testFiles.slice(0, remainingSlots)
      );
    }
